...
int main(int argc, char* argv[]) {
    ...
    chrono::SetChronoDataPath("D:/qsq/chrono800/data/");              // change the default data loading path.
    chrono::vehicle::SetDataPath("D:/qsq/chrono800/data/vehicle/");              // change the vehicle data path
 ...
 // 
    ...
    while (vis->Run()){
        ...
        if (time < INIT_WAIT_TIME) {
            static int wait_cnt = 0;
            if (wait_cnt++ % 1000 == 0) {
				std::cout << "Waiting for: " << INIT_WAIT_TIME << "s to begin. Current time: " << time << " s." << std::endl;
            }
        ...
        if (step_number % 1000 == 0)
            std::cout << "Real-world time: " << step_number/1000 << "s." << std::endl;
