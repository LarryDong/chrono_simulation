...
function generateJson(outputPath, params,a)
    ...
    % 动态生成文件名
    fileName = sprintf('%d-A%dF%dV2-2024.12.23.json', params.Vibration_mode, a, params.IMU_force_freq);
...
outputPath = 'D:\lab\项目\振动对slam影响\matlab\json\json_output\accY_2024.12.23\Acc_12.23'; 
...
params.base_M = 10;
params.corner_M = 10;
params.chtime_end = 125;

params.Vibration_mode = 3;
...
for f = 1:f_l
    for a = 1:a_l
        params.IMU_force_amp = round(name{a+1, f+1}, 3); % 保留三位小数的数值
        params.IMU_force_freq = freq_list(f);
